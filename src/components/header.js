...
export default function Header() {
  const [headerLinkClassNames, setHeaderLinkClassNames] = useState(
    "no-underline font-bold text-white text-base px-2 py-2 font-yekan border-solid border-b-2 border-transparent hover:border-white hidden md:block"
  );
  ...
  const handleHamburgerClick = () => {
    setHeaderLinkClassNames(
      "no-underline font-bold text-white text-base px-2 py-2 font-yekan border-solid border-b-2 border-transparent hover:border-white"
    );
    setHeaderLinks(
  ...
  return (
    <div className="flex flex-row-reverse justify-between items-center w-full h-14 bg-twitter-blue px-10">
      <Link className="text-2xl font-bold text-white font-yekan" href="/">
        توییترفا
      </Link>
